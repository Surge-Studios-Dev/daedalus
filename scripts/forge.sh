...
echo
bold "Share links + referrals (SHARING.md - ships default-on)"
echo "  - Deploy hosting with the backend: firebase deploy --only hosting,functions"
echo "    (serves /s /i /c rewrites + the .well-known deep-link files)."
echo "  - AASA: replace TEAMID in hosting/public/.well-known/apple-app-site-association"
echo "    and add Associated Domains (applinks:<host>) to ios/Runner/Runner.entitlements."
echo "  - App Links: paste the SHA256 fingerprint(s) into .well-known/assetlinks.json and"
echo "    add the autoVerify intent filter for the host(s) in AndroidManifest.xml."
echo "  - Custom domain (optional): set SHARE_LINK_BASE in backend/.env; the *.web.app"
echo "    host must stay registered forever (old links). DNS host field wants only the"
echo "    subdomain part ('go'), not the FQDN. Remember: backend/.env is deploy-machine"
echo "    state - record changes in this checklist's project notes."
echo "  - Rewards: set REVENUECAT_SECRET in backend/.env; until then referral days bank"
echo "    as creditDays and drain when the key lands. Copy the launcher icon to"
echo "    hosting/public/brand-mark.png (LAUNCH-TODO(brand-mark))."
echo "  - Verify on the recipient's REAL device, both directions, both platforms."
echo
